...
The use of multidimensional scaling in spatial statistics is not new. The earliest mention is \cite{Sampson:1992wm} who model spatial covariance functions by computing a distance measure based on the observed spatial covariances (calculated from multiple observations at each location), then project the points using MDS (although only into 2 dimensional space) before smoothing. Comparison with other previous work is covered in Sections 2 and 5.
...
The combination of an un-modified null space, the opacity of the penalty meaning and $O(n^3)$ computational cost are of some concern for practical work. For these reasons it seems worthwhile to investigate alternative ways of using the within-area distance idea, avoiding these difficulties.
